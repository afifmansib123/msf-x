...
const ProfileForm = ({ data, date, userID, handleEdit }) => {
  ...
  const onSubmit = async (userData) => {
    ...
    if (
      regex.test(userData.contact_number) &&
      (userData.contact_number.length == 11 ||
        userData.contact_number.length == 14)
    ) {
    ...
    if (okay == true) {
      ...
      try {
        const response = await axios.patch(
          `${process.env.NEXT_PUBLIC_BG_API}user/profile/update/${userID}/`,
          formData,
          {
            headers: {
