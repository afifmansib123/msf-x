...
function BikeApproveLog(props) {
    const useStyles = makeStyles(styles);
    const classes = useStyles();
    const handleClickOpen = (bikeRecord) => {
        props.callback(bikeRecord.bikeId);
    };

    const onClickedHistoryBtn = (bikeid) => {
        props.historyBtnClicked(bikeid);
    };
    ...
    var showedData = props.tableData.map((value, index) => {
        return [
            ...
            <div className="grid grid-cols-2">
                <div className="col mr-4 font-medium">Bike Name</div>
                <div className="col-md-8">{value?.bikeName || "-"}</div>

                <div className="col-3 mr-4 font-medium">Merchant Name</div>
                <div className="col-md-8">{value?.Merchant_Name || "-"}</div>

                <span className="mr-4 font-medium">Bike Maker</span>
                <span>{value?.Bike_Maker || "-"}</span>

                <span className="mr-4 font-medium">Bike Country</span>
                <span>{value?.manufacturerData?.maker_country || "-"}</span>

                <span className="mr-4 font-medium">Bike Model</span>
                <span>{value?.Bike_Model || "-"}</span>
            ...
            return (
                <div className="grid grid-cols-1">
                    <div className="flex flex-row">
                        ...
                        <div className="">
                            <h1 className="font-medium text-lg my-2">{data?.bikeName || "-"}</h1>
                            <div className="mt-10">
                            ...
                            <Button
                                variant="outlined"
                                onClick={() => {
                                    onClickedHistoryBtn(data.bikeId);
                                }}
                            >
                                History
                            </Button>
    ...
    return (
        <>
            ...
            <span>
        Merchants have submitted the bikes to be approved. Once these are approved, they will be shown on the public
      </span>
            <Card>
                <CardBody>
...
BikeApproveLog.defaultProps = {
    tableHeaderColor: "warning",
};

BikeApproveLog.propTypes = {
