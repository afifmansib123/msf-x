...
function DetailBikeLog(props) {
    ...
    const handleSubmit = async (type) => {
        if (type === "cancel") {
            await router.push(`/admin/bike-approval/`);
            return;
        }
        ...
        try {
            if (type === "approve") {
                await handleApprove(reason, id, parseInt(router.query.id), parseInt(props.bike.id));
                await router.push(`/admin/bike-approval/`);
            } else {
                await handleReject(reason, id, parseInt(router.query.id), parseInt(props.bike.id));
                await router.push(`/admin/bike-approval/`);
            }
        } catch (e) {
            console.error(e)
...
async function handleApprove(review_string, approval_id, bike_id, approvelogId) {
...
async function handleReject(review_string, approval_id, bike_id, approvelogId) {
    const response = await fetch("/api/approve-log/review", {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
